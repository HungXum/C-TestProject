...
void malloc_mem_control_block()
{
    ...
    //但是实验得出我的电脑是8字节对齐,因为申请121个字节的空间,如果是16字节对齐,应该得出整个block的size = 144,但是结果是136;而且很奇怪的一个点是block的header只有size = 8字节,并没有glibc中的pre_size??????
    //--------------------
    //.     .            .
    //.     .            .
    //--------------------
...
void cur_chunk_data(void* mem)
{
    ...
    printf("mem = %#x, pchunk = %#x,size before chunksiez = %d, size = %zu\n", mem, pchunk, pchunk->size, chunksize(pchunk));
}
