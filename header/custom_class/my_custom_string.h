...
#ifndef HEADER_CUSTOM_CLASS_MY_CUSTOM_STRING_H_
#define HEADER_CUSTOM_CLASS_MY_CUSTOM_STRING_H_
...
class String {
 ...
 public :
    ...
    explicit String(const std::string& stdStr) {
        std::cout << "String (const std::string& stdStr)\n";
        if (stdStr.data() != nullptr) {
            size_t size = stdStr.length();
            char *temp = new char[size + 1];
            strncpy(temp, stdStr.data(), size);
            temp[size] = '\0';
...
#endif  // HEADER_CUSTOM_CLASS_MY_CUSTOM_STRING_H_
